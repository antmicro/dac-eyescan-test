...
def read_back_from_char(dev, voltage_off, phase_off, bit_select, is_r0=True):
    ...
    readback_decoded = decode_bitbang(readback)[BITBANG_OUTPUT_BIT][27 if is_r0 else 25:][::2][:194][::-1]
...
def configure_receiver_block(dev, receiver_block):
    ...
    jtag_write_read(dev, encode_bitbang_ir(ws_cfg().to_binary()[::-1] + ("0" if receiver_block == 1 else "")))
    select_command(dev, COMMANDS[receiver_block]["SELECT_CFG"])
    jtag_write_read(dev, encode_bitbang_ir(ws_cfg(core_we_head=True, core_we=True, char_we=True, core_we_tail=True).to_binary()[::-1] + ("0" if receiver_block == 1 else "")))
    select_command(dev, COMMANDS[receiver_block]["SELECT_CORE_INPUTS"])
    jtag_write_read(dev, encode_bitbang_ir(ws_core(enpll=True, mpy=20, enrx=True, buswidth=2, term=1, eq=1, enoc=True, cfg_ovr=True).to_binary()[::-1] + ("0" if receiver_block == 1 else "")))
...
def parse_args():
    parser = argparse.ArgumentParser(
                    ...
                    epilog='A script to connect to the DAC38J8x using JTAG and perform eyescan test')
    parser.add_argument('-o', '--output', required=True, type=pathlib.Path, help="output file path")
    parser.add_argument('-d', '--device', type=int, default=0, help="FTDI device number")
